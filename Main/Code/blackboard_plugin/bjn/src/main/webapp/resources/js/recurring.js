(function($, _app) {
	...
	function populateForm() {
		if ($('#repeat').prop('checked')) {
		    ...
		    switch($('input[name="recurssion_ending"]:checked').val()) {
		        ...
		        case 'OCCURRENCES':
		            ...
		            break;
		        case 'ON':
		            $('input[name="recurrencePattern.endDate"]').val(Date.parse($('#dp_endDate_date').val()));
		            $('input[name="recurrencePattern.recurrenceCount"]').val('0');
		            break;
		    }
	    ...
	    // Start time / End time
		var unixtimeStart = moment($('#start').val(), "YYYY-MM-DD HH:mm:ss").unix() * 1000;
		var unixtimeEnd = moment($('#end').val(), "YYYY-MM-DD HH:mm:ss").unix() * 1000;
